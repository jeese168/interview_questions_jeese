...
- **不回表**（覆盖索引），但**只有 a 走了索引**，b = '1' 因隐式类型转换导致索引失效，c 也无法使用索引。

**第2句：** `select a, b, c from table where a > 1 and c > 1 and b > 1;`

- **不回表**（覆盖索引），但**只有 a > 1 走了索引**，因为 a 是范围查询后，b 和 c 都无法再利用索引进行条件过滤（最左前缀原则中断）。
...
- 隐式类型转换相当于对列加了函数，会导致索引失效
- 范围查询后，后续列无法继续使用索引的条件过滤功能
