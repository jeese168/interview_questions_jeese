...
第一是**SQL Interface**，接收SQL命令，返回结果。主要起到一个承上启下的作用为两边进行翻译（SQL语句 `->` MySQL理解的格式    存储引擎执行完的结果`->` 用户理解的表格式）。
...
存储引擎负责实际的数据存储和操作，如InnoDB和MyISAM等。**热插拔的本质是：不同表可以指定不同存储引擎（如一张表用 InnoDB，另一张用 MyISAM）**
...
> 但实际物理逻辑执行可能不会按照这个来，查询优化器在允许的情况下可能会进行优化，比如逻辑上WHERE是在ON的后面，但是如果查询优化器判断WHERE过滤大量不必要的连接，那就会提前执行，这叫「**谓词下推**」。
...
##### Q：左连接时，把右表连接条件写在 ON 后面和 WHERE 后面，结果一样吗？
A：不一样。  
**ON** 是连接条件，决定右表怎么匹配左表。如果右表不满足 ON 条件，左表行保留，右表补 NULL。  
**WHERE** 是筛选条件，是对连接后的结果集进行清洗。如果把右表的条件写在 WHERE 里，那些补 NULL 的行会被过滤掉，**LEFT JOIN 会在逻辑上退化为 INNER JOIN**。

> 但对于现代查询优化器而言，虽然逻辑上 WHERE 是后过滤，但现在的数据库优化器都有**谓词下推**功能。  
> 如果是 Inner Join，写在 ON 和 WHERE 性能通常没区别。  
> 如果是 Left Join，把右表条件写在 WHERE 里导致逻辑变成了 Inner Join，数据库反而可能优化得更快（因为数据量少了，或者可以交换驱动表了），**但前提是这么做符合业务逻辑，否则结果就是错的。**
...
- CHAR是**固定长度的字符串类型**，**定义时需要指定固定长度，存储时会在末尾补足空格。** CHAR适合存储长度固定的数据，如固定长度的代码、状态等，存储空间固定对其存储，所以性能也比较高，对于短字符串效率较高，最大长度是255字符。
...
- `UNION`：会对合并的结果 **去重** 并且按照第1个字段进行**排序**（默认 `DISTINCT`），性能较 `UNION ALL` 低，因为需要额外的去重和排序计算。
- `UNION ALL`：直接合并多个 `SELECT` 结果，不去重而且也不排序，性能较好，适用于不需要去重的情况。
...
这个查询会对customers表的每一行（外查询），检查orders表中是否存在对应的订单（内查询），如果找到第1条就返回第一结果，随后根据匹配条件查找剩下的。

也就是说`EXISTS` 优先外查询而子查询用于判断外查询是否匹配，它不关心子查询返回什么数据，只关心是否有结果。
...
**第一点关于性能方面**，
1. 如果子查询结果集较小且不频繁变动，`IN` 仅不需要便利，而且直观易懂性能更好。
2. 但表很大时（比如有100万条记录），`EXISTS` 的性能优于 `IN`。IN 会先执行子查询，获取所有customer_id，可能产生大量临时数据，但EXISTS 则是对每个customers表的记录，查找是否存在对应订单，找到一条就停止。
3. **如果两个表都很大**，还要看索引。EXISTS 走的是**外层驱动内层**，所以内层查询的索引很关键；IN 走的是**内层驱动外层**，所以外层表的索引很关键。

**第二点关于宽泛方面，**
`IN` 支持常量查询`IN (1,2,3)`，而`EXISTS`不支持。
- `IN (SELECT ...)` 如果子查询结果包含 NULL，IN 会正常返回，而`NOT IN` 子查询结果包含 NULL会导致整个查询无结果。
- `EXISTS` 不受 NULL 影响，它只关心“有没有数据返回”。
...
具体来说，第一，InnoDB引擎提供了对事务的支持，即可以进行ACID（原子性、一致性、隔离性、持久性）属性的操作。Myisam存储引擎是不支持事务的。

第二，InnoDB引擎采用了行级锁定的机制有**更好的并发性能**，而Myisam存储引擎只支持表锁，锁的粒度比较大。

此外InnoDB引擎通过 redolog 日志实现了崩溃恢复，可以在数据库发生异常情况（如断电）时，通过日志文件进行恢复，保证数据的持久性和一致性。而其他引擎一般是不支持崩溃恢复的。

以及InnoDB**支持外键约束（维护表间一致性）** 多表关联场景（如 “订单表” 关联 “用户表”）中，InnoDB 可通过外键约束禁止 “订单存在但用户已删除” 这类数据不一致的情况；而 MyISAM 不支持外键，只能靠业务代码手动维护一致性，容易出错。

最后InnoDB**聚簇索引优化查询效率**，InnoDB 的主键索引（聚簇索引）会直接存储数据，查询时找到索引就等于找到数据，无需额外回表；而 MyISAM 的索引和数据分开存储，查完索引还得再查数据，效率更低，尤其对大表查询优化明显。
...
- t_order.ibd，t_order 的表数据会保存在这个文件。表数据既可以存在共享表空间文件（文件名：ibdata1 全局唯一）里，也可以存放在独占表空间文件（文件名：表名字.ibd）。这个行为是由参数 innodb_file_per_table 控制的，若设置了参数 innodb_file_per_table 为 1，则会将存储的数据、索引等信息单独存储在一个独占表空间，从 MySQL 5.6.6 版本开始，它的默认值就是 1 了，因此从这个版本之后， MySQL 中每一张表的数据都存放在一个独立的 .ibd 文件。
...
**除此之外索引还针对索引列的顺序来进行排序，所以如果涉及到索引列的顺序，不需要额外的排序操作。**

#### 索引的优缺点？

索引的优点就是提高了查询速度，并且支持排序。
...
使用前缀索引的目的是为了减少索引占用的存储空间，提升查询效率（因为单个索引记录的空间变小导致一个磁盘快的，容纳的记录变得更多了，减少了IO）。
...
因此，在使用联合索引查询时，必须要**遵循最左匹配原则**才能使其生效，**因为有查询优化器，所以最左字段在 where 子句的顺序并不重要。**
...
##### Q：联合索引ABC（都是int类型），有以下三个查询语句汇总回表吗？哪些走了索引？

>select a, b, c from table where a = 1 and b = '1' and c = 1;
  select a, b, c from table where a > 1 and c > 1 and b > 1;
  select a, b, c from table where a = 1 and b = 1 and c = 1;


**第1句：** `select a, b, c from table where a = 1 and b = '1' and c = 1;`

- **不回表**（覆盖索引），a 走了索引，b = '1' 虽然是隐式转换，但是属于输入为字符串，转换为数字可以走索引，c 也可使用索引。

**第2句：** `select a, b, c from table where a > 1 and c > 1 and b > 1;`

- **不回表**（覆盖索引），但**只有 a > 1 走了索引**，因为 a 是范围查询后，b 和 c 都无法再利用索引进行条件过滤（最左前缀原则中断），但是可以走索引下推。

**第3句：** `select a, b, c from table where a = 1 and b = 1 and c = 1;`

- **不回表**（覆盖索引），**a、b、c 都走了索引**，完全利用联合索引，性能最优。

**关键点：**

- 字符串`->`数字隐式类型转换索引仍然有效，但是数字`->`字符串类型这种隐式转换会失效
- 范围查询后，后续列无法继续使用索引的条件过滤功能，但是可以走索引下推
- 三个查询都不回表是因为 select 的列都在索引中（覆盖索引）
...
- **左或左右模糊匹配**：`like '%xx'` 或 `like '%xx%'`
- **对索引列使用函数**：如 `WHERE YEAR(date_column) = 2024`
- **对索引列进行表达式运算**：如 `WHERE id + 1 = 5`
- **隐式类型转换**：如字符串列与数字比较 `WHERE varchar_col = 123`
- **违反最左前缀原则**：联合索引 ABC 中直接查 `WHERE B = 1`
- **OR 条件中有非索引列**：`WHERE indexed_col = 1 OR non_indexed_col = 2`

>联合索引中后面字段索引失效还包括，范围查询后的列他们的索引也是失效的
...
- **先问业务**  
    "这条 SQL 是 OLTP 实时查询还是 OLAP 报表？数据量级、并发、时效要求分别是多少？"  
    **解释：** OLTP（在线交易）要求毫秒级响应，索引要精准；OLAP（数据分析）可以容忍秒级延迟，更关注吞吐量，优化思路完全不同。
- **再拿执行计划**  
    "我会用 EXPLAIN / ANALYZE 把 type、rows、Extra 三列抓出来，先看是不是全表扫或临时排序。"  
    **解释：** type=ALL 就是全表扫描最慢，rows 显示扫描行数越少越好，Extra 出现"Using filesort"或"Using temporary"说明有额外排序或临时表开销。
- **三看数据分布**  
    "把 where 列的基数、选择性、空值率跑一遍，低基数列加索引往往得不偿失。"  
    **解释：** 基数=去重后的值数量，选择性=基数/总行数，比如性别列只有2个值（低基数），加索引也没用，反而浪费空间和写入性能。
- **四定索引策略**  
    "用'覆盖索引 + 最左前缀 + 范围后置'三原则拼候选 idx，再用虚拟列/函数索引解决隐式转换或函数包列问题。"  
    **解释：** 覆盖索引让查询不回表，最左前缀保证联合索引生效，范围查询放最后避免截断索引；遇到 WHERE YEAR(date)=2024 这种，可以加函数索引或建虚拟列来解决。
- **五做压测回滚**  
    "先在影子库用 sysbench 或线上采样做 A/B，确认 CPU/IO 下降 20% 以上再灰度，同时保留老索引 48h 可回滚。"  
    **解释：** 新索引不一定比旧的好，要用压测工具模拟真实流量验证效果，灰度上线后监控资源消耗，万一变慢能快速回滚老索引避免事故。
...
![img](/文档图片/1717479903616-831081f3-45bc-4436-a066-2702266abfce.png)
...
>  MVCC（多版本并发控制）其作用主要就是在缓解了并发读写之间的冲突，使得不需要完全通过悲观锁的方式来进行互斥读写隔离


##### 3. 事务隔离级别

Mysql提供了多种事务隔离级别，包括读未提交、读已提交、可重复读和串行化。**事务隔离级别** 是 MySQL 在 SQL 层的全局配置，底层的话仍然是基于mvc和锁机制来进行实现，通过控制事务之间的隔离程度，以避免数据不一致的问题。
...
可重复读指一个事务执行过程中看到的数据同是一致的，符合同一个视图，即同一事务多次读取的结果始终与事务开始时的快照数据一致，是 **MySQL InnoDB 引擎的默认隔离级别**。

在可重复读隔离级别下，通过 MVCC 机制**避免了脏读和不可重复读和大部分幻读现象**，MVCC没有解决执行过程中视图修改的幻读现象，但大部分的幻读现象都解决了，基本够用。

可以通过显示的在查询语句后面加上 `FOR UPDATE` 通过引入 **Next-Key Lock** 锁，彻底解决了幻读问题。因此，InnoDB 在可重复读隔离级别下，不会发生脏读、不可重复读或幻读现象（取决于用户的事物逻辑实现）。
...
MVCC多版本并发控制是基于undo log来进行实现的，允许多个事务同时读取同一行数据，而不会彼此阻塞，每个事务看到的数据版本按照预先定义的事务隔离级别规则读取到相应的版本。
...
**当执行插入、更新、删除操作，需要先对表加上「意向独占锁」**，然后对该记录加独占锁。**意向锁的目的是为了快速判断表里是否有记录被加锁**。是行锁和表锁的 “桥梁”—— 比如要加表级排它锁，先判断有没有意向排它锁，不用逐行检查行锁，提升锁判断效率。
...
> "MySQL InnoDB 本身只有悲观锁机制,如记录锁、元数据锁、Next-Key Lock 等,都是共享锁或排他锁。
> 
> 所谓的'MySQL 乐观锁'其实是**应用层实现的并发控制策略**,通过在表中增加 version 字段或时间戳,在更新时通过 WHERE 条件判断数据是否被修改过:
> 
> ```sql
...
> ```
> 
> 如果 affected_rows = 0,说明数据已被修改,更新失败。这种方式不需要数据库加锁,适合读多写少、冲突较少的场景。"
...
准确的来说是InnoDB会在**任意事务请求锁被阻塞时触发死锁检测**，采用了一种懒检测的方式，通过构建等待图（至于操作系统资源分配图）来寻找循环依赖，当事务试图获取锁但需要等待时，也会触发死锁检测。
...
此外，开发者还可以通过SQL语句来进行主动地判断当前存储引擎是否存在死锁。
...
- Using where，表示查询索引没办法完全过滤数据，需要在SQL层来进行额外的过滤。
...
基本上分为三步走，慢SQL的定位 -> 分析慢SQL的问题（几方面分析） -> 问题的解决方案就是优化
##### 1. 慢SQL定位
第1种方法是，首先要开启慢查询日志，慢查询日志中会记录下查询时间超过设定值的SQL。
第2种方法的话，就是利用已有的基础设施，监控业务SQL的执行超过阈值了，马上报警通过办公软件、邮件或者电话的方式立即告警责任人。

##### 2. 分析慢SQL的问题（几方面分析）
利用上面所说的explain来对这条定位的SQL分析
1. 通过`key` 来判断师否索引没有使用或使用不当
2. 再次`type` 通过再次确定当前查询优化器判断走的的索引是否合适
3. 通过 `extra` 判断是否可能走了额外的文件排序
4. 结合上面这些字段和业务SQL本身来判断是查询优化器选择索引还是SQL能够进一步优化
5. 通过`rows`这个字段来排查是不是数据量太大。
6. 除此之外，也要考虑到并发量的问题，可以通过基础设施或者自己写一个SQL审计系统，来记录一下这个业务SQL在一定时间内执行的数量如果较大频繁，那可能是并发导致的。
...
1. 严禁使用 select `*` ： 增加查询分析器解析成本；无用字段传输增加网络消耗，**应该根据当前的业务情况撰写最合适的SQL**
2. join 多表查询时，使用数据量小的表驱动数据量大的表(连接查询是匹配，一般是用左表的一条记录，然后去右表查)
3. 当合并多表数据时，如果业务数据允许出现重复的记录，推荐使用union all 代替union，因为union会自动去重，更加耗时。
4. 分页优化，当深度分页时避免使用`LIMIT n, m`，而是在where时直接过滤进行游标分页。



**索引优化**
1. 限制每张表上的索引数量，建议单张表索引不超过 5 个（太多索引会导致写操作速度越慢，因为要维护）。
2. 如果使用联合索引，区分度最高的列和最频繁使用的列放在最左侧
3. 一般会对在 where 后的字段建索引，也会对在Order By、Group By 后的字段建索引，减少额外排序。
4. 如果用到的索引不准确，使用`force index` 强制走自己指定的索引



**表结构优化和设置优化**
1. 反范式设计，对于经常需要年表查询的特定字段在表中保留一些冗余字段，避免连表操作或多表联查
2. 不要用字符串存储日期，使用时间戳
3. 适当提高内部排序缓冲区的大小，避免因覆盖索引和排序操作无法兼顾时候而不得已使用排序时使用了外部磁盘排序大大影响性能



**架构优化（解决并发导致及数据量过大的问题）**
1. **读写分离**： 将读操作和写操作分离到不同的数据库实例，提升数据库的并发处理能力。

2. **分库分表**： 使用分库分表中间件，比如说ShardingSphere将数据分散到多个数据库实例或数据表中，降低单表数据量，提升查询效率。

3. **缓存机制**： 使用 Redis 等缓存中间件，将热点数据缓存到内存中，减轻数据库压力。

4. **分布式数据库：** 使用云服务器厂商提供的分布式数据库TDSQL等，或自建集群部署开源的优质分布式数据 TiDB 支持高并发和大数据量。



##### Q：分页优化具体是什么意思？有什么常见的解决思路吗？

A：使用 `LIMIT n, m` 时，MySQL 需要从第 0 条记录开始扫描 n + m 条数据，然后丢弃前 n 条，只返回后 m 条。显然效率很低。

而分页优化常见的解决思路，一般的话就是游标分页，当每次选择翻页的时候客户端或者前端会把当前页面的最后一条主键值或者其标识一并传输过来，利用主键或索引的有序性，通过 `WHERE id > 某个值` 来定位查询起点，只扫描后续的行，加快查询速度。


##### Q：你的方案适合‘下一页’这种场景。但如果我的前端页面支持输入框，用户直接输入要访问 **第 10000 页**，这时候你客户端没有 `last_id`，怎么办？
A：那我的解决方案其实是延迟关联，深度分裂的根本问题是 MySQL 蠢在“回表后再丢弃”，那我们就**让它只在索引上跑，把 ID 拿到了，丢弃完不需要的 ID 后，只拿着最后那 10 个 ID 去回表**。实现思路也很简单，就是使用内连接的方式。
```SQL
SELECT t1.* FROM orders t1
INNER JOIN (
    -- 这里只查 ID，利用 覆盖索引 (Covering Index)
    -- 即使扫描 10万行，也全是内存操作，不回表，速度极快
    SELECT id 
    FROM orders 
...
`master（binlog dump thread）`主要负责Master库`binlog`有数据更新的时候，会按照`binlog`格式，将更新的binlog发送给连接上来的从库。
...
A：首先，必须**严谨判断**是真宕机还是网络分区（防止脑裂）。
确认故障后，紧急操作流程如下：
1.  **止损：** 尝试停止所有从库的 IO 线程，防止从库从旧主库接收错误数据。
2.  **选主：** 比较所有从库，选择 **GTID 最全**（GTID指的是server_id拼接上trx_id分布式情况下唯一标识一个事物或 Binlog 位置最新）且延迟最小的从库作为新主。
3.  **提权：** 将新主库的 `read_only` 关闭，开放写入。
4.  **重连：** 修改其他从库指向新主。这里如果开了 **GTID**，直接开启 `AUTO_POSITION` 就能自动对接，非常快。
5.  **引流：** 修改中间件（如 ShardingSphere）或 VIP 指向，恢复业务写操作。


##### Q：你知道自动处理主节点宕机的方案吗？*
A：关于自动化高可用，虽然我没有在生产环境从零搭建过，但我了解一套目前比较主流且现代化的思路，就是结合 **MGR + ShardingSphere**。

**MGR 通过 Paxos 协议解决“数据一致性”和“自动选主”问题。**

这套方案的运行机制如下：

1. **MGR 内部选主**：当主节点故障，MGR 集群内部通过 Paxos 协议投票，自动选出新的主节点（保证数据不丢）。
2. **ShardingSphere 感知**：ShardingSphere 通过内置的 `Database Discovery` 功能，实时探测 MGR 的集群角色变化。
3. **自动切换路由**：一旦探测到主节点变了，ShardingSphere 自动更新内部路由，把写流量切到新主节点。
...
#### TiDB 的三大核心组件（**TiDB Server, PD, TiKV**）分别负责什么？


#### 为什么 TiDB（或者说 RocksDB）要选 LSM-Tree 而不是 B+ 树？
