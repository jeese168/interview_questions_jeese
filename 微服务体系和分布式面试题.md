...
*   **阶段一：准备阶段 (Prepare Phase) —— 抢占领导权**
    *   Proposer 生成一个提案编号 $N$（必须递增），向所有的 Acceptor 发送 `Prepare(N)` 请求。
    *   **潜台词**：“我要开始提议了，编号是 N，你们如果有接受过比 N 小的提案，告诉我；如果没有，请承诺以后不再接受比 N 小的提案。”

*   **阶段二：接受阶段 (Accept Phase) —— 锁定值**
    *   如果 Proposer 收到了超过半数 Acceptor 的回复（Promise），它就会发起真正的提案 `Accept(N, Value)`。
    *   **潜台词**：“既然大家给面子，那我就把值定为 Value 了，请大家把这个值记录下来（持久化）。”
    *   Acceptor 收到后，如果没有违背之前的承诺，就会接受这个值。
...
Paxos 可以算得上是分布式协议的鼻祖但是他有两个问题：
- 实现非常困难，连谷歌和雅虎的资深工程师表示实现Basic Paxos 相当困难。
...
1. **版本向量 (Vector Clocks)**：用来跟踪数据被修改的历史和版本，帮助系统在网络恢复后智能地合并不同分支的修改。
2. **CRDTs (Conflict-free Replicated Data Types，无冲突复制数据类型)**：这是一种高级数据结构。它的数学特性保证了对数据的修改操作在任何节点上执行，无论它们的**顺序如何颠倒**，最终的结果都是一样的。CRDTs 将冲突解决的逻辑内置到了数据结构本身的设计中，使数据在网络恢复后能够**自动、无冲突地合并**。
...
PING包含发送方哈希槽分配，同时也请求被ping的 Redis Cluster 主节点发送PONG消息，回应给他。 
