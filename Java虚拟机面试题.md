...
元空间取代永久代主要是为了避免OOM异常。因为通常使用PermSize和MaxPermSize（永久代设置参数）设置永久代的大小就决定了永久代的上限，但是不是总能知道应该设置为多大合适, 如果使用默认值很容易遇到OOM错误。
...
字面量是直接写在代码中的固定值，包括基本数据类型的字面量（10、3.14、true、'a'）和字符串字面量（"hello"）。

对于String字面量在编译时JAVA便会在字节码文件记录字面量，在运行时加载到常量池中。
对于基本数据类型字面量，它们作为字节码指令的一部分存在，使用时会被加载到操作数栈，如果赋值给变量，变量存储在栈帧的局部变量表中。
字面量作用在于在编译时和运行时由编译器和 JVM 进行高效处理，尤其是字符串字面量可节省性能（通过复用常量池中的对象减少资源消耗）和节省内存（通过共享字符串实例避免重复创建消耗存储）。
...
如 `WeakHashMap`，这个map存储的键值对对象，键是被`WeakHashMap`对象通过弱引用的方式来进行管理，值是强硬用，但是当键回收之后，`WeakHashMap`对象会自动回收这个键对应的值（所以说不太会出现所有的logo的那种情况，只要保证直对象不反过来对他的健对象有引用就好），而适用于一些临时映射的情况，主动将键的强引用断开之后，下次gc就会回收对应的对象，适合技术大佬深度的优化。
...
直接内存溢出的异常信息为**OutOfMemoryError: Direct buffer memory**，直接内存是堆外内存，属于操作系统的本地内存，但可以用于JVM管理操作系统的IO交互，尤其是在Java的NIO类分配直接内存为缓冲区使用时，但分配的内存总额超过`-XX:MaxDirectMemorySize` 限制时，会发生溢出。
...
故若大量使用static关键字修饰的变量可能造成内存泄露。另一个典型的内存泄露场景是使用静态集合（static修饰的集合）长期保存大量对象引用，这些对象即使不再需要，也会持续占用内存空间。这种情况下，垃圾回收器无法回收这些对象，导致内存资源无法有效释放，随着时间推移可能会消耗大量内存。
...
**类型指针**有指向元空间中类对应的元数据的指针等（普通的实力对象直接通过类型指针访问获得元数据）。
...
开发者可通过特定需求自行实现**自定义类加载器**（即继承`ClassLoader`类），来从网络、加密文件和数据库来加载类，自定义类加载器可提高应用的的灵活性和安全性，是Java动态性的一个重要体现。自定义类加载器的父类是**系统类加载器**且由其加载。
...
1. **满足不同依赖需求**：Tomcat可以同时运行多个Web应用，每个应用可能使用不同版本的相同库（比如Spring、Hibernate等）。如果严格遵循双亲委派，所有应用将被迫使用同一版本的库，这会导致版本冲突。
2. **实现热部署**：也实现了Tomcat可以在不重启服务器的情况下重新加载或更新Web应用。只需要卸载类加载器及加载的类随后再进行装载，就可以一个应用被重新部署。
...
- 准备：为类的静态字段分配内存，并设置默认的初始值（即0、`false`、`null`）。（静态字段存储的class对象之中，加载时创建的class对象只是个半成品，到现在才正式填充）
...
1. 栈中的局部变量引用 
2. 静态变量引用 
3. 常量引用（编译时就能确定值的字面量）：如字符串常量池中的对象、包装类`Integer num = 1;`
4. 本地方法栈中的引用 
5. 被同步锁持有的对象（synchronized关键字）
...
同时也会出现多标问题，也就是已经标记过的黑色对象在并发执行过程中已经没有被引用了，也就是多标，这个处理不处理都行反正下次GC就会判断出来回收。
...
垃圾回收算法是Java虚拟机自动管理堆中对象内存释放的机制，核心解决的是**手动内存管理的痛点**。
1. **避免内存泄漏与内存溢出**：手动管理内存时（如 C/C++），若忘记释放无用内存（内存泄漏）或重复释放（野指针），会导致内存耗尽（溢出），GC 通过自动识别并回收垃圾对象，从根源减少这类问题。
2. **降低开发复杂度，提升效率**：开发者无需关注 “何时分配内存、如何释放内存” 等底层细节，可专注于业务逻辑编写，大幅减少因内存管理导致的 bug，提升开发效率。
...
（CMS垃圾回收器通过并发优化了一下标记和清除效率，但是又引入了浮动垃圾的问题）
...
| ----------------- | ---------- | ------------ | ---------- | --------------- | ---------------------- | ----------- |
| Serial            | 串行         | 新生代          | 复制算法       | 简单高效，单线程        | 单核 Client 模式           | 单线程全程 STW   |
| ParNew            | 并行         | 新生代          | 复制算法       | 多线程优化响应速度       | 多核 Server + CMS 组合     | 多线程并行全程 STW |
| Parallel Scavenge | 并行         | 新生代          | 复制算法       | 吞吐量优先           | 后台计算                   | 多线程并行全程 STW |
| Serial Old        | 串行         | 老年代          | 标记-整理      | 单线程后备方案         | Client 模式或 CMS 失败后备    | 单线程全程 STW   |
| Parallel Old      | 并行         | 老年代          | 标记-整理      | 吞吐量优先           | 与 Parallel Scavenge 组合 | 多线程并行全程 STW |
...
A：CMS 垃圾回收器可能会因为内存碎片问题、浮动垃圾问题、CPU 资源不足等原因失败。当老年代的内存空间存放不下浮动垃圾时（垃圾回收还赶不上线程创建垃圾的速度）或因为碎片太多没有空间分配时，就会导致并发清理模式失败发生全带堆内存回收，此时 CMS 会退化为线程来进行垃圾回收。

Serial和Serial Old是单线程的同时，在清理过程中会有全程STW，所以时延较长。

**G1也是因为垃圾回收，还赶不上线程申请分配内存的速度就会退化成Parallel Old**
...
#### minorGC、majorGC、fullGC的区别，什么场景触发full GC（排除新的垃圾回收器ZGC/Shenandoah）
...
fullGC的**作用范围**是整个堆空间，甚至是元空间。当触发Full GC时，**所有回收器都会退化为完全STW的标记-整理算法**。
