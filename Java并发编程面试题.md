...
**JMM**是Java内存模型（Java Memory Model）的简称，它规定了变量在内存中的读写规则，线程如何与主内存交互，如何保证线程间的数据一致性，特别是在不同线程之间如何共享数据。

JMM的核心目的是确保在并发环境中多线程操作共享变量时的**可见性**、**原子性**和**有序性**。
...
线程是进程的一部分，一个进程在其执行的过程中可以产生多个线程，而且线程是是CPU执行和调度的基本单位。与进程不同的是同类的多个线程共享进程的**堆**、**方法区**（本地内存区域的元空间）和直接内存资源，但每个线程有自己的**程序计数器**、**虚拟机栈**和**本地方法栈**。
...
#### 什么是线程上下文切换？什么时候出现线程上下文切换？

线程要保存在执行过程中自己的运行条件和状态（即上下文），比如上文所说到过的程序计数器，栈信息等。**上下文切换**需要保存当前线程的上下文，留待线程下次占用 CPU 的时候恢复现场。并加载下一个将要占用 CPU 的线程上下文。
...
**可见性：** 一个线程对资源（主内存）的修改是对于其他线程及时可见的。在Java中使用synchronized和volatile这两个关键字来确保可见性。
...
**在沉睡中停止**：先将线程sleep进入WAITING或TIMED_WAITING状态，然后调用interrupt标记中断状态，interrupt会将阻塞状态的线程中断。会抛出中断异常，达到停止线程的效果，**注：Blocking状态不会仅仅只设置标志位，不会响应**
...
`Thread.interrupt()`仅设置线程的中断状态，若该被中断的线程中的run方法中定义了相关处理策略后，根据预先定义好的策略检测中断状态来决定是否要停止当前正在执行的任务如上面的返回和抛出异常，如果没写不会有任何发生。

**但如果该线程正在执行低级别的可中断方法**（如`Thread.sleep()`、`Thread.join()`或`Object.wait()`），这些方法会自动检测中断状态，响应统一是1.解除阻塞 2.**抛出InterruptedException异常**。
如果执行的是`LockSupport.park()`，当线程被 `interrupt()` 时，`park()` 会立即返回（解除阻塞），不会抛出异常。
...
- 当线程退出 `synchronized` 代码块时，**会自动释放锁，唤醒锁池（entryList）中的一个线程**

**注：释放锁+唤醒**是原子的，实现太过底层，记住这个就行。
...
- 线程不安全则表示在多线程环境下，因为静态条件，对于同一份数据，多个线程同时访问时可能会导致数据被覆盖或丢失、程序崩溃或异常和和单线程执行结果不一致。
...
活锁指的是线程或进程或某个服务持续改变其状态，但无法取得实际进展的情况。与死锁不同，活锁中的线程并没有被阻塞，它们仍在积极执行，但由于互相干扰或让步，导致无法完成实际工作。

一个生活中的例子是：两个人在走廊相遇，双方同时向左闪避，发现还是会撞到，于是又同时向右闪避，如此反复，双方都在不断移动但无法通过走廊。

解决方法很简单，对于线程或者进程设置优先级比如ID小的优先，就像规定走廊相遇时，年龄大的人先通过。
来说的话出现活锁现象一般是指分布式系统，一般采用指数级退避算法来减少冲突。
...
通过volatile关键字一般实现的是Java线程安全中的可见性和有序性，volatile关键字应用于变量，确保其他线程对于该变量观察得到的值是主存的最新值，而不是本地寄存器的副本。
...
Java中的JUC库（`java.util.concurrent`）定义了一种锁也叫做读写锁，可重入的同时有三种特点：**读读共享、读写互斥、写锁排他** 

读写锁通常用于读取远多于写入的情况，以提高并发性。同时的话也适用于一种资源需要互斥处理，其中某些操作这份资源的操作可以共享进行操作的情况就可以分配给他共享锁即读锁，提高整体并发性。
...
A：**可中断锁**：获取锁的过程中可以被中断，不需要一直等到获取锁之后 才能进行其他逻辑处理。`ReentrantLock` 就属于是可中断锁，能响应interrupt。

**不可中断锁**：一旦线程申请了锁，就只能等到拿到锁以后才能进行其他的逻辑处理。 `synchronized` 就属于是不可中断锁，对interrupt操作仅仅设置中断状态值为true。





#### 这些锁在什么场景下使用？怎么在实践中用锁的？*
...
用自旋锁获取不到手的情况一直利用**CAS操作**反复尝试获取锁而自旋的行为，适合于临界区执行比较短的情况。
否则一直尝试自旋，反复消耗cpu资源导致性能还不如重量级锁
...
synchronized关键字在底层是基于Java中对象的监视器锁，每一个监视器锁都有线程ID和锁状态（status）两个字段，更底层是基于os互斥锁的。

**当线程尝试获取锁时，首先会看锁状态（status）：**

若锁状态（status）为0，则使用CAS操作将线程ID替换为自己的线程ID

若锁状态（status）不为0，但线程ID是自己的线程ID，若是可重入锁则将锁状态（status）自增加一，进入同步代码执行。

**当先线程释放锁时：**

若为可重入锁将锁状态（status）自减1，不为您继续持有锁执行，只有锁状态（status）为0才会完全释放锁，当时唤醒锁池entrylist的其中的一个线程。
...
A：全都是基于AQS实现，通过 `LockSupport.park()`来将线程变为waiting状态实现锁住一个线程。
...
非公平锁通过允许"插队"，让刚好到达的线程有机会直接获得锁，避免了不必要的线程唤醒和调度开销，从而提高了整体吞吐量。但这是以牺牲公平性为代价的。
...
![img](1719982991053-c983c4b8-7c5f-4e2d-a90d-5b42ebd533d5.png)
...
**关于强引用和弱引用的详见JVM面试题：[[Java虚拟机面试题#引用类型有哪些？有什么区别？]]**
...
1. **PriorityBlockingQueue（优先级队列）**，任务可以按照**优先级**进行排序，优先级高的任务先执行，**无法指定最大容量。**
   - 适用于**需要优先处理某些关键任务**的场景，如定时任务调度。
1. **DelayQueue（延迟队列）**，任务按照**指定的延迟时间**执行，**无法指定最大容量。**
   - 适用于**定时任务、缓存自动清理**等场景。
...
shutdown ()调用后，线程池状态变为SHUTDOWN，正在执行任务的线程会继续执行并且也会去取阻塞队列里面的内容继续执行，空闲的线程会被中断，不能添加新任务到线程池中，否则会抛出异常，在所有任务执行完后线程池会关闭。
