...
- **HashSet**：基于HashMap实现，元素无序且不允许重复。适合用于需要检查唯一性的数据集合。
- **LinkedHashSet**：基于LinkedHashMap实现，元素按插入顺序排列，且不允许重复。
- **TreeSet**：基于TreeMap实现，元素按自然顺序或自定义的比较器排序，且不允许重复。


>注：
Set 接口的实现类通常委托给对应的 Map 实现
 利用 Map 的 key 不重复特性来实现 Set 的元素唯一性
 这样可以复用 Map 的底层数据结构和算法 
...
- **缓冲区**：数据按顺序进出的缓冲处理
2. 双端队列类型
- **滑动窗口算法**：需要从窗口两端添加/删除元素
- **当作栈使用**：Deque 可以高效地当栈用（比 Stack 类更好）
...
- **ConcurrentSkipListMap**
   基于跳表（SkipList）算法实现的线程安全有序 `Map`。跳表是一种能在对数预期时间内完成搜索、插入、删除等操作的数据结构，通过多个指向其他元素的跳跃链接实现高效查找。虽然在性能上不如**ConcurrentHashMap**，**但是支持需要顺序的场景。**

| 集合类                   | 数据结构       | 特点        | 实现方式             | 适用场景      | 性能                    |
| --------------------- | ---------- | --------- | ---------------- | --------- | --------------------- |
| ConcurrentHashMap     | 哈希表+链表+红黑树 | 支持高并发、分段锁 | CAS+synchronized | 高并发读写不需顺序 | 极好（平均复杂度O(1)最坏O(n)）   |
| ConcurrentSkipListMap | 跳表         | 有序Map     | CAS              | 需要排序的并发场景 | 好（平均复杂度O(logn)最坏O(n)） |
...
- **缺点**：允许修改元素, 但不适合修改集合元素，尤其是在遍历过程中进行删除操作时效率较低。
...
| 增强for循环      | 所有Collection | - 语法简洁<br>- 代码可读性好<br>- 不会出现索引错误                                  | - 无法获取索引<br>- 无法修改集合结构<br>- 无法同时遍历多个集合 | - 简单遍历场景<br>- 不需要修改集合<br>- 仅需要读取元素    |
...
| Stream API   | 所有Collection | - 支持函数式操作<br>- 支持并行处理<br>- 可以进行复杂操作                               | - 简单操作性能开销较大，不适合简单遍历                   | - 复杂数据处理<br>- 需要链式操作<br>- 需要并行处理      |
...
此外，在遍历时修改集合结构（除了通过Iterator/ListIterator的remove方法）可能会抛出ConcurrentModificationException异常。

Stream API虽然强大，但在简单场景下可能会带来不必要的性能开销，但复杂操作和并行处理时可能更优。
...
A：`ConcurrentModificationException` 是 Java 中的一个运行时异常，主要是为了检测集合的**结构性并发修改**，无论是多线程还是单线程环境下的不当操作都可能触发。它属于 **java.util** 包中的异常类型。主要是为了防止并发修改集合时的潜在不一致问题。集合会维护一个 **modCount** 计数器，记录集合被修改的次数。如果在迭代时检测到这个计数器发生了变化，就会抛出 `ConcurrentModificationException`。



一般地，使用**以上的所有方法**遍历集合时，如果在遍历过程中，集合的结构（例如元素的增加、删除或更新操作）被修改了，且没有通过 `Iterator` 提供的 `remove` 或 `add` 方法进行修改，就会抛出 `ConcurrentModificationException` 异常。
普通for循环，而且针对于list这种类型的集合进行修改元素结构，因为没有用到迭代器所以不会报这个并发异常。
以上只针对于java.until包下面的普通集合类型，JUC 集合不能一概而论。
大多数JUC 集合（Concurrent系列、 Copy on write系列）在遍历的时候允许用他们自身方法修改，因为设计思路不一样。


**Q：fail-fast 和 fail-safe 机制是什么？它们与 ConcurrentModificationException 有什么关系？**

A：**fail-fast** 和 **fail-safe** 是两种不同的错误处理设计哲学，直接决定了集合是否会抛出 `ConcurrentModificationException`。

**Fail-Fast 机制（快速失败）**

**设计思想**：遇到异常情况立即停止运行，避免产生更严重的后果。

**适用集合**：java.util 包下的普通集合（ArrayList、HashMap 等）

**实现原理**：

- 集合维护 `modCount` 计数器，迭代器保存 `expectedModCount`
- 每次 `next()` 都检查：`modCount != expectedModCount`
- 不相等就抛出 `ConcurrentModificationException`

**Fail-Safe 机制（安全失败）**

**设计思想**：即使面对并发修改也能继续运行，保证系统稳定性。

**适用集合**：JUC 包下的并发集合（ConcurrentHashMap、CopyOnWriteArrayList 等）

**实现原理（CopyOnWriteArrayList为例）**：

- **写时复制**：修改时创建数据副本，迭代基于旧数据继续
- **弱一致性迭代器**：不检查 modCount，允许并发修改
- **代价**：迭代时遍历看到的可能不是最新数据

一句话就是，

- **fail-fast**：宁可报错也不允许不一致（java.util 集合）
- **fail-safe**：宁可数据滞后也要保证运行（JUC 集合）
...
**迭代器的好处**：它提供了一种统一的方式来遍历不同类型集合（如 List、Set、Map 等），而无需暴露底层数据结构的实现细节。
...
1. 统一接口：通过 Iterator 和 Iterable 接口标准化集合遍历方式。
2. 内部类实现：每个集合类 通过内部类 结合自身的数据结构定制迭代逻辑（如数组索引、链表指针）。
...
**1. 查找成本** 你说得对，LinkedList 的插入/删除虽然是 O(1)，但前提是你已经有了节点引用。实际使用中你还得先找到位置，这就是 O(n) 的遍历成本。

**2. 内存局部性**

- ArrayList 数据连续存储，**缓存友好**
- LinkedList 节点分散在堆中，缓存命中率低
- 现代 CPU 的多级缓存对连续内存访问有巨大优化

**3. JVM 优化**

- JVM 对数组操作有专门的字节码指令优化
- 连续内存更容易被 JIT 编译器优化
- 数组边界检查等也有特殊优化

**实际开发中默认选 ArrayList**，除非有**特殊需求且经过性能测试证明 LinkedList** 更好。

**LinkedList 适用的罕见场景**：

- 实现队列/栈（但现在更推荐 ArrayDeque）
...
* **高效的并发读取**，特别适合读取远多于写入的场景（比如白名单校验，数据变动不频繁，但是读的操作非常多）。可以在多个线程间高效并发读取数据。
...
JDK 1.7 HashMap 采用数组 + 链表的数据结构而且还是用头插法，多线程背景下，如果两个线程几乎同时进行扩容操作，**扩容时的链表迁移过程中，因为是头插法导致 链表指针的错误更新**，从而造成链表变成一个环形链表。导致程序运行时进入无限循环，导致系统卡死或内存泄漏。
...
而且由于新增的1bit是0还是1可以认为是随机的，因此resize的过程，均匀的把之前的冲突的节点分散到新的bucket（哈希槽）了。
...
负载因子太低会导致大量的空哈希槽浪费空间，负载因子太高会导致大量的碰撞，降低性能。

负载因子 0.75 是基于 泊松分布 的统计规律：当负载因子为 0.75 时，哈希冲突的概率显著降低。  所以0.75 是经过大量测试的折中值，可达到最佳平衡。
...
而且JDK 1.7 `ConcurrentHashMap` 中固定默认构造是16个分段 Segment，而且之后永远不会改变，**但除了第1个分段Segment 对象在对象实例创建的时候就创建了其他的 Segment 对象是懒加载的，需要的时候才会创建分配资源**。
...
HashTable所有的方法都加了锁（synchronized关键字）来保证线程安全，但是效率非常的低下，当一个线程访问同步方法，另一个线程也访问的时候，就会陷入阻塞或者轮询的状态。

##### 3.扩容过程

HashTable扩容是在添加时判断，然后一次性全部扩容完成，有比较大的性能影响。

扩容时会同时维护新旧两个数组。查找元素时先查旧数组，没找到再查新数组；写入操作默认写到新数组中。

在查找和写入过程中，会有一个volatile标识指导线程们进行迁移哪一些哈希槽的元素。通过 CAS 原子性利用这个标识地获取一个迁移范围，然后将这个范围内的哈希槽元素从旧数组迁移到新数组。

迁移过程复用原有的并发控制机制（CAS + synchronized），确保迁移时的线程安全。这样既不阻塞正常的读写操作，又能让多个线程协作完成扩容，提高了并发性能。



综合来看的话，concurrentHashMap具有良好的伸缩性和性能，hashtable属于淘汰的东西。
...
- **Hashtable**：不具备很好的伸缩性。元素增加虽然数组容量增加同时分散哈希冲突，但是锁的粒度完全没有改变，所以线程数增加，性能变化不大反而创建资源消耗了。
- **ConcurrentHashMap**：设计时充分考虑了伸缩性，尤其是在 **水平伸缩（线程数的增加，提高处理性能）** 上表现得很好。在 JDK 8 以后，`ConcurrentHashMap` 实现桶级别的细粒度的锁，线程数增加，性能近似线性增长。
...
| **扩容触发条件**        | 当元素数量超过负载因子（默认 0.75）时触发扩容                   | 超过负载因子，时触发扩容（扩容比例为 2n + 1）                    | 当负载因子超过一定阈值时触发扩容，且扩容时性能优化                                                            |
